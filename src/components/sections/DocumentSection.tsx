...
// --- TYPE DEFINITIONS ---
interface StrapiFileAttributes {
   url: string;
   ext: string;
   mime?: string;
   size?: number;
...
export default function DocumentSection({ data }: DocumentSectionProps) {
   ...
   const getFileInfo = (fileData: StrapiFile | null | undefined) => {
      if (!fileData) return { url: null, ext: "" };
      const rawUrl = fileData.url || fileData.data?.attributes?.url || fileData.attributes?.url || null;
      const ext = fileData.ext || fileData.data?.attributes?.ext || fileData.attributes?.ext || "";
   ...
   return (
      <section className="w-full py-12 md:py-20 bg-gray-50 min-h-screen">
                           ...
                           <div key={group.id || gIndex} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                              ...
                              <div className="divide-y divide-gray-50">
                                 {(group.files || []).map((file, fIndex) => {
                                    ...
                                    let fullUrl = null;
                                    if (url) {
                                       fullUrl = url.startsWith("http") ? url : `${STRAPI_URL}${url}`;
                                    }
