...
export default function HeroSlider({ data }: HeroSliderProps) {
   ...
   return (
      <section
         className="relative w-full h-[500px] md:h-[600px] lg:h-[700px] bg-gray-900 overflow-hidden group -mt-20 lg:-mt-24"
      ...
      >
         {slides.map((slide: SlideItem, index: number) => {
            ...
            // Fetch URL Gambar
            const rawImg = slide.image?.url || slide.image?.data?.attributes?.url;
            const imageUrl = getStrapiMedia(rawImg);

            // Fallback Alt Text
            const altText = slide.title || "Hero Banner";
