...
export default function StaffCard({ data, globalBannerUrl, icons, locale = "id" }: StaffCardProps) {
  if (!data) return null;

  const rawData = data as unknown as { attributes?: StaffAttributes } & StaffAttributes;
  ...
  const rawPhotoUrl = (photoData as { attributes?: { url: string } })?.attributes?.url || (photoData as { url?: string })?.url;
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-xl transition-shadow duration-300 group flex flex-col h-full">
      ...
      <div className="h-20 w-full relative bg-green-50 shrink-0">
        {bannerSrc ? (
          <>
            <Image src={bannerSrc} alt="Card Background" fill className="object-cover" />
      ...
      <div className="px-6 pb-6 relative flex flex-col flex-grow">
        ...
        <div className="-mt-16 mb-3 relative w-32 h-32 rounded-full border-4 border-white shadow-md overflow-hidden bg-gray-200 shrink-0">
          {imageUrl ? (
            <Image src={imageUrl} alt={name || "Staff"} fill className="object-cover" />
          ) : (
            <div className="w-full h-full flex items-center justify-center bg-gray-300 text-gray-500 text-xs text-center">No Photo</div>
          )}
        </div>
        ...
        <div className="space-y-1 mb-4 flex-grow">
          ...
          {category === "administrasi" ? (
            <>
              <p className="text-sm font-semibold text-green-700 mt-1">{adminData?.position || "-"}</p>
              <p className="text-xs text-gray-500 font-medium mt-2">NIP: {nip || "-"}</p>
              <p className="text-sm text-gray-700 mt-1">
                {/* Update: Menggunakan label dinamis & sejajar */}
                <span className="font-medium text-gray-500 mr-1">{rankLabel}</span>
                {adminData?.rank || "-"}
              </p>
            </>
          ) : (
            <>
              <p className="text-xs text-gray-500 font-medium mt-1">NIP: {nip || "-"}</p>
        ...
        {category === "akademik" && academicData && (
          <div className="flex items-center gap-4 pt-4 border-t border-gray-100 mt-auto">
            ...
            {academicData.sinta_url && (
              <Link href={academicData.sinta_url} target="_blank" className="hover:opacity-80 transition-opacity flex items-center" title="Sinta">
                {sintaIconSrc ? (
                  <Image src={sintaIconSrc} alt="Sinta Logo" width={40} height={40} className="h-10 w-auto object-contain" />
                ) : (
            ...
            {academicData.scopus_url && (
              <Link href={academicData.scopus_url} target="_blank" className="hover:opacity-80 transition-opacity flex items-center" title="Scopus">
                {scopusIconSrc ? (
                  <Image src={scopusIconSrc} alt="Scopus Logo" width={40} height={40} className="h-10 w-auto object-contain" />
                ) : (
            ...
            {academicData.google_scholar_url && (
              <Link href={academicData.google_scholar_url} target="_blank" className="hover:opacity-80 transition-opacity flex items-center" title="Google Scholar">
                {scholarIconSrc ? (
                  <Image src={scholarIconSrc} alt="Scholar Logo" width={40} height={40} className="h-10 w-auto object-contain" />
                ) : (
